...
export async function storagePut(
  ...
  if (r2) {
    ...
    return { key, url: await getSignedUrl(r2.client, new GetObjectCommand({ Bucket: r2.bucket, Key: key }), { expiresIn: 3600 }) };
  }
...
export async function storageGetSignedUrl(relKey: string): Promise<string> {
  const r2 = getR2Config();
  if (r2) return getSignedUrl(r2.client, new GetObjectCommand({ Bucket: r2.bucket, Key: normalizeKey(relKey) }), { expiresIn: 3600 });
  const { forgeUrl, forgeKey } = getForgeConfig();
  const key = normalizeKey(relKey);
